...
class mainclass(ttk.Frame):
    def __init__(self, notebook, packet):
        ...
        self.KCLI_Label = ttk.Label(self.leftframe, justify=LEFT, text="A Command Line interface for downloading\n"
        ...
        self.KCLIgui_Label = ttk.Label(self.rightframe, justify=LEFT, text="A Grafical User interface for the KCLI Kernel\n\n"
                                                                        f"Version: {packet.packversion}\n"
                                                                        f"Developer: Konstantin Ehmann\n")

        self.KCLIgui_Label.pack(anchor=NW)

        ttk.Button(self.leftframe, text="Creator Website", command=lambda: os.system("start http://ksite.ddns.net")).pack(anchor=NW, pady=2)
        ttk.Button(self.leftframe, text="Creator Github",
                   command=lambda: os.system("start https://github.com/Olikonsti")).pack(anchor=NW, pady=2)
        ttk.Button(self.leftframe, text="KCLI Github",
                   command=lambda: os.system("start https://github.com/Olikonsti/KCLI")).pack(anchor=NW, pady=2)
